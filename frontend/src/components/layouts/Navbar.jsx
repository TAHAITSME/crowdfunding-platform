...
import { Bell, MessageCircle, Search, ChevronDown, LogOut,
         User, Settings, Bookmark, X, Moon } from 'lucide-react'
...
export default function Navbar() {
  ...
  // Ferme dropdown au changement de route
  ...
  const Avatar = ({ size = 'sm' }) => {
    ...
    return (
      <div className={`${s} rounded-full overflow-hidden bg-green-100 flex items-center justify-center shrink-0`}>
        {avatarUrl
          ? <img src={avatarUrl} alt={displayName} className="w-full h-full object-cover" />
          : <span className="text-green-700 font-black">{initials}</span>
        }
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between gap-3">
        ...
        <div className="flex-1 max-w-sm relative hidden md:block">
          ...
          <input
            ...
            placeholder="Rechercher des posts, profils..."
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 text-gray-400 hidden sm:block transition-transform duration-200 ${dropdown ? 'rotate-180' : ''}`} />
            </button>
            ...
            {dropdown && (
              <div className="absolute right-0 top-[calc(100%+8px)] bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 w-60 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                ...
                {[
                  ...
                  <button
                    key={to}
                    onClick={() => { navigate(to); setDropdown(false) }}
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition rounded-xl mx-auto"
                  >
